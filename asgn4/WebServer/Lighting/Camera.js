class Camera {
  constructor() {
    this.fov = 60;
    this.eye = new Vector3([0, 0.8, -2.4]);
  ...
  pitchUp(alpha) {
    ...
    const right = Vector3.cross(new Vector3(), forward, this.up).normalize();
    // build a rotation about the right axis
    const R = new Matrix4();
    R.setRotate(alpha, right.elements[0], right.elements[1], right.elements[2]);
    // rotate forward and up
    const fPrime = R.multiplyVector3(forward);
    this.at = new Vector3(this.eye.elements).add(fPrime);
    this.up = R.multiplyVector3(new Vector3(this.up.elements));
    // update stored pitch & view matrix
    this.pitch += alpha;
    this.updateView();
  }
